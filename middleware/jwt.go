...
func JWTAuth() gin.HandlerFunc {
	return func(context *gin.Context) {
		...
		// 用户UID
		uid := utils.ParseStringToInt64(context.Request.Header.Get("uid"))
		// 字段为空 请求非法
		if auth == "" {
			context.JSON(TokenError, gin.H{
				"code": TokenMissHeader,
				"msg":  "Request header has no authorization field",
			})
			context.Abort()
			return
		}
		...
		//	解析token
		j := NewJWT()
		if status, err := j.ParseToken(kv[1], uid); err != nil {
			context.JSON(TokenError, gin.H{
				"code": status,
				"msg":  err.Error(),
			})
